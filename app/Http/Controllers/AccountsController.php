    ...
    public function index()
    {
        ...
        if($role->hasPermissionTo('account-index')){
            ...
            foreach ($lims_account_all as $account) {
                ...
                $debit  = $returns + $expenses + $payrolls + $sent_money_via_transfer;
                $account->balance = ($credit - $debit) - $payment_sent;
            }
    ...
    public function accountStatement(Request $request)
    {
        ...
        if($data['type'] == '0' || $data['type'] == '2') {
            $credit_list = Payment::whereNotNull('sale_id')
                            ...
                            ->whereDate('payment_at', '>=' , $data['start_date'])
                            ->whereDate('payment_at', '<=' , $data['end_date'])
                            ->select('payment_reference as reference_no', 'sale_id', 'amount', 'payment_at as created_at')
                            ->get();
        ...
        if($data['type'] == '0' || $data['type'] == '1') {
            $debit_list = Payment::whereNotNull('purchase_id')
                            ...
                            ->whereDate('payment_at', '>=' , $data['start_date'])
                            ->whereDate('payment_at', '<=' , $data['end_date'])
                            ->select('payment_reference as reference_no', 'purchase_id', 'amount', 'payment_at as created_at')
                            ->get();
