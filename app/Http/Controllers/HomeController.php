...
class HomeController extends Controller
{
    ...
    public function dashboard()
    {
        ...
        if(Auth::user()->role_id == 5) {
            ...
            $lims_sale_data = Sale::with('warehouse')->whereNull('deleted_at')->where('customer_id', $customer->id)->where('sale_type', '!=', 'opening balance')->orderBy('created_at', 'desc')->get();
        ...
        while($start < $end)
        {
            ...
            if(Auth::user()->role_id > 2 && cache()->get('general_setting')->staff_access == 'own') {
                $sale_amount = Sale::whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->where('user_id', Auth::id())->sum(DB::raw('grand_total / exchange_rate'))->whereNull('deleted_at');
    ...
    public function recentSale()
    {
        if(Auth::user()->role_id > 2 && cache()->get('general_setting')->staff_access == 'own')
        {
            $recent_sale = Sale::join('customers', 'customers.id', '=', 'sales.customer_id')->select('sales.id','sales.reference_no','sales.sale_status','sales.created_at','sales.grand_total','sales.exchange_rate','sales.user_id','customers.name')->orderBy('id', 'desc')->whereNull('sales.deleted_at')->where('sales.user_id', Auth::id())->where('sale_type', '!=', 'opening balance')->take(5)->get();
            return response()->json($recent_sale);
        }
        else
        {
            $recent_sale = Sale::join('customers', 'customers.id', '=', 'sales.customer_id')->select('sales.id','sales.reference_no','sales.sale_status','sales.created_at','sales.grand_total','sales.exchange_rate','customers.name')->orderBy('id', 'desc')->whereNull('sales.deleted_at')->where('sale_type', '!=', 'opening balance')->take(5)->get();
            return response()->json($recent_sale);
        }
    }
